...
def elbo_rvae(data, p_mu, p_sigma, z, q_mu, q_t, model, beta, device):
    if model._mean_warmup:
        ...
        log_pxz = Normal(p_mu, p_sigma).log_prob(data).sum(-1).to(device)
        log_qzx = log_bm_krn(z, q_mu, q_t, model, device)
        log_pz = log_bm_krn(z, pr_mu.expand_as(z), pr_t, model, device)
        KL = log_qzx - log_pz
