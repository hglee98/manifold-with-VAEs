...
class PiecewiseLinearFit:
    ...
    def get_new_initial_knots(self, method='kmeans'):
        ...
        if method == 'kmeans':
            kmeans = KMeans(n_clusters=self.nKnots,
                            max_iter=300).fit(self.data_to_fit)
    ...
    def order_knots(self, knots, method='nearest'):
        ...
        for i in range(1, len(knots)):
            ...
            if method == 'nearest':
                ...
                next_idx = fhf.find_smallest_dist_idx(
                    ord_knots[i - 1], rem_knots)
    ...
    def fit_data(self, fit_params):
        ...
        def cost_fn(flat_knots, penalty_params=fit_params):
            ...
            loop_knots = fhf.loop_knots(np.reshape(
                flat_knots.copy(), (self.nKnots, self.nDims)))
            ...
            if penalty_params['penalty_type'] == 'none':
                ...
                cost = np.mean(
                    dists) + penalty_params['len_coeff'] * self.tot_len(loop_knots)
            return cost
    ...
    def global_to_local_coords(self):
        ...
        t_int_idx = cut(self.tt, bins=t_bins,
        ...
        t_rsc = (self.tt - t_bins[t_int_idx]) / \
            (t_bins[t_int_idx + 1] - t_bins[t_int_idx])
...
def fit_manifold(data_to_fit, fit_params):
    ...
    init_knots = fitter.order_knots(
        unord_knots, method=fit_params['knot_order'])
