...
class spike_counts:
    ...
    def single_intvl_dict(self, state, interval_num=0):
        ...
        if self.count_type == 'binned':
            ...
            interval_angles = np.array(
                self.spike_data[state][interval]['avg_angles'])
            ...
            # Check types
            interval_angles = np.array(
                self.spike_data[state][interval]['angles'][::jump])
    ...
    def get_spike_matrix(self, state, interval_num='all'):
        ...
        count_matrix = np.array([spike_counts[cell]
                                for cell in self.cell_ids]).T
